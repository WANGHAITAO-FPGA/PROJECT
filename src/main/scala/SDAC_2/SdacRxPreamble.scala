...
case class SdacRxPreamble(datawidth : Int) extends Component{
  val io = new Bundle{
  ...
  val startDelimiter = B"x00F1F2F300000002"
...
case class SdacRxSimpleBus(addrwidth : Int, datawidth : Int) extends Component{
  val io = new Bundle{
    val input = slave(Stream(Fragment(Bits(datawidth bits))))
