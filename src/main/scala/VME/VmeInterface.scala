...
case class Vme(datawidth : Int) extends Component{
  ...
  val vme_area = new ClockingArea(ClockDomain(io.clk,io.reset)){
    ...
    val addr_pass_data1 = (((io.vme.am === B"6'x3C"))&&io.vme.lword&&io.vme.iack)
    ...
    //val addr_pass_data = addr_pass_data1&& (!io.vme.as)&&(io.vme.addr(19 downto 16) === (((~io.gap)<<1).resize(4 bits)))&&(vme_realaddr(24) === ~io.gap4)&&(io.vme.addr(22 downto 20)===B"3'x6")
    val addr_pass_data = addr_pass_data1&& (!io.vme.as)&&(io.vme.addr(18 downto 15) === (((~io.gap).asUInt-2).asBits.resize(4 bits)))&&(io.vme.addr(19) === ~io.gap4)
...
case class VME_REG(datawidth : Int) extends Component{
  val io = new Bundle{
    ...
    val original_data = out Vec(Bits(32 bits),22)
  }
  noIoPrefix()
  val busslave = Apb3BusInterface(io.apb,(0x0000, 1000 Byte))

  val vme_data_temp = Vec(Reg(Bits(32 bits)) init(0),6)

  val original_data_temp = Vec(Reg(Bits(32 bits)) init(0),22)

  val vme_data = Vec(Reg(Bits(32 bits)) init(0),6)

  val original_data = Vec(Reg(Bits(32 bits)) init(0),22)

  val refresh = Vec(Reg(Bool()) init(False),22)
...
/*****************************VME REG ****************************************************/
  if(datawidth == 16){
    ...
    vme_data(0) := RegNextWhen(VME_Reg0##VME_Reg1,Delay(My_Reg1.hitDoWrite,4,init = False))
    ...
    vme_data(1) := RegNextWhen(VME_Reg2##VME_Reg3,Delay(My_Reg3.hitDoWrite,4,init = False))
    ...
    vme_data(2) := RegNextWhen(VME_Reg4##VME_Reg5,Delay(My_Reg5.hitDoWrite,4,init = False))
    ...
    vme_data(3) := RegNextWhen(VME_Reg6##VME_Reg7,Delay(My_Reg7.hitDoWrite,4,init = False))
    ...
    vme_data(4) := RegNextWhen(VME_Reg8##VME_Reg9,Delay(My_Reg9.hitDoWrite,4,init = False))
...
//    SENSOR_Reg11 := io.sensor_data(11)
  ...
  trigger := refresh(10)
  when(trigger.rise()){
    vme_data_temp := vme_data
    original_data_temp := original_data
  }
  io.vme_data := vme_data_temp
  io.original_data := original_data_temp

//  val ila_probe = ila("1",io.vme_data(0),io.vme_data(1),io.vme_data(2),io.vme_data(3),io.vme_data(4),io.vme_data(5),io.original_data(0),io.original_data(1),io.original_data(2),
//    io.original_data(3),io.original_data(4),io.original_data(5),io.original_data(6),io.original_data(7),io.original_data(8),io.original_data(9),io.original_data(10))
}
...
//    io.data <> vme_apb.io.data
    ...
    io.original_data <> vme_reg.io.original_data
  }
}
