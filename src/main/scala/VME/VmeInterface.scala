...
case class Vme() extends Component{
  val io = new Bundle{
    val clk = in Bool()
    val reset = in Bool()
...
case class VME_APB() extends Component{
  val io = new Bundle{
    val lwr = in Bool()
    val lrd = in Bool()
    val laddr = in Bits(15 bits)
    val data = master(TriState(Bits(16 bits)))
    val apb = master(Apb3(Apb3Config(addressWidth = 15,dataWidth = 16)))
  }
  noIoPrefix()
...
case class VME_REG() extends Component{
  val io = new Bundle{
    val apb = slave(Apb3(Apb3Config(addressWidth = 15,dataWidth = 16)))
...
case class VME_TOP() extends Component{
  val io = new Bundle{
    val clk = in Bool()
    val reset = in Bool()
    val vme = master(VmeInterface())
  ...
  val vme_area = new ClockingArea(ClockDomain(io.clk,io.reset)){
    ...
    val vme_reg = new VME_REG
    vme_reg.io.apb <> vme_apb.io.apb
...
object VME_TOP extends App{
  SpinalVerilog(InOutWrapper(new VME_TOP))
}
