...
public:
    ...
    Wrapper_1(const char * name){
      ...
      waveEnabled = true;
      signalAccess[0] = new CDataSignalAccess( top.data_in );
      signalAccess[1] = new CDataSignalAccess( top.data_out );
      ...
      top.trace(&tfp, 99);
      tfp.open((std::string("D:\\SCALA\\SRIO\\.\\simWorkspace\\Noise_Filter/") + name + ".vcd").c_str());
      #endif
      this->name = name;
    }
    ...
    virtual ~Wrapper_1(){
      ...
      #endif
      #ifdef COVERAGE
      VerilatedCov::write((("D:\\SCALA\\SRIO\\.\\simWorkspace\\Noise_Filter/") + name + ".dat").c_str());
      #endif
    }
