...
public:
    ...
    Wrapper_1(const char * name){
      ...
      waveEnabled = true;
      signalAccess[0] = new CDataSignalAccess( top.log_rst_q );
      signalAccess[1] = new CDataSignalAccess( top.log_clk );
      signalAccess[2] = new IDataSignalAccess( top.s_axi_tx_tdata );
      signalAccess[3] = new CDataSignalAccess( top.s_axi_tx_tlast );
      signalAccess[4] = new CDataSignalAccess( top.s_axi_tx_tvalid );
      signalAccess[5] = new CDataSignalAccess( top.s_axi_tx_tready );

      #ifdef TRACE
      Verilated::traceEverOn(true);
      top.trace(&tfp, 99);
      tfp.open((std::string("D:\\SCALA\\SRIO\\.\\simWorkspace\\GTP_TEST/") + name + ".vcd").c_str());
      #endif
      this->name = name;
    }
    ...
    virtual ~Wrapper_1(){
      ...
      #endif
      #ifdef COVERAGE
      VerilatedCov::write((("D:\\SCALA\\SRIO\\.\\simWorkspace\\GTP_TEST/") + name + ".dat").c_str());
      #endif
    }
