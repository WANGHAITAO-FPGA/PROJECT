...
public:
    ...
    Wrapper_1(const char * name){
      ...
      waveEnabled = true;
      signalAccess[0] = new CDataSignalAccess( top.input_valid );
      signalAccess[1] = new CDataSignalAccess( top.input_ready );
      signalAccess[2] = new CDataSignalAccess( top.input_payload_last );
      signalAccess[3] = new IDataSignalAccess( top.input_payload_fragment );
      ...
      signalAccess[8] = new CDataSignalAccess( top.timer_tick );
      signalAccess[9] = new CDataSignalAccess( top.clk );
      signalAccess[10] = new CDataSignalAccess( top.reset );
      signalAccess[11] = new CDataSignalAccess( top.led );

      #ifdef TRACE
      Verilated::traceEverOn(true);
      top.trace(&tfp, 99);
      tfp.open((std::string("D:\\SCALA\\SRIO\\.\\simWorkspace\\FiberTest/") + name + ".vcd").c_str());
      #endif
      this->name = name;
    }
    ...
    virtual ~Wrapper_1(){
      ...
      #endif
      #ifdef COVERAGE
      VerilatedCov::write((("D:\\SCALA\\SRIO\\.\\simWorkspace\\FiberTest/") + name + ".dat").c_str());
      #endif
    }
