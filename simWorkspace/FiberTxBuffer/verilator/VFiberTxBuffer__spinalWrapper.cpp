...
public:
    ...
    Wrapper_1(const char * name){
      ...
      waveEnabled = true;
      signalAccess[0] = new CDataSignalAccess( top.io_push_stream_valid );
      signalAccess[1] = new CDataSignalAccess( top.io_push_stream_ready );
      signalAccess[2] = new CDataSignalAccess( top.io_push_stream_payload_last );
      signalAccess[3] = new CDataSignalAccess( top.io_push_stream_payload_fragment );
      signalAccess[4] = new CDataSignalAccess( top.io_pop_stream_valid );
      signalAccess[5] = new CDataSignalAccess( top.io_pop_stream_ready );
      signalAccess[6] = new IDataSignalAccess( top.io_pop_stream_payload );
      signalAccess[7] = new CDataSignalAccess( top.io_pop_last );
      signalAccess[8] = new CDataSignalAccess( top.pushclk_clk );
      signalAccess[9] = new CDataSignalAccess( top.pushclk_reset );
      signalAccess[10] = new CDataSignalAccess( top.popclk_clk );
      signalAccess[11] = new CDataSignalAccess( top.popclk_reset );

      #ifdef TRACE
      Verilated::traceEverOn(true);
      top.trace(&tfp, 99);
      tfp.open((std::string("D:\\SCALA\\SRIO\\.\\simWorkspace\\FiberTxBuffer/") + name + ".vcd").c_str());
      #endif
      this->name = name;
    }
    ...
    virtual ~Wrapper_1(){
      ...
      #endif
      #ifdef COVERAGE
      VerilatedCov::write((("D:\\SCALA\\SRIO\\.\\simWorkspace\\FiberTxBuffer/") + name + ".dat").c_str());
      #endif
    }
